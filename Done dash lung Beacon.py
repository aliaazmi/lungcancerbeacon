...
df = pd.read_csv('https://raw.githubusercontent.com/aliaazmi/data/main/Lung_cancer.csv')
...
df_filterd2 = df[df['Type of Lung Cancer2'].isin(['NSCLC-Adeno', 'NSCLC-Squamous', 'SCLC', 'Sarcoma', 'Lymphothelial',
                                                   'Malignant mesothelioma'])]
fig2 = px.pie(df_filterd2, values='Count', names='Type of Lung Cancer2',
             title='<b>Type of Lung Cancer (n=405)</b>',
             labels ='Type of Lung Cancer2', color_discrete_sequence=px.colors.sequential.Brwnyl)
fig2.update_traces(textposition='inside', textinfo='percent+label')


df_filterd1 = df[df['Stage2'].isin(['I', 'II', 'III', 'IV'])]
fig1 = px.pie(df_filterd1, values='Count', names='Stage2',
             title='<b>Stage for Lung Cancer Pt (n=380)</b>',
             labels ='Stage2', hole=.3, color_discrete_sequence=px.colors.sequential.Magenta)
